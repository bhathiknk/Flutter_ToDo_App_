...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MemoFilesRecover(),
    );
  }
}

class MemoFilesRecover extends StatefulWidget {
  @override
  State<MemoFilesRecover> createState() => _MemoFilesScreenState();
}

class _MemoFilesScreenState extends State<MemoFilesRecover> {
  late int userId;
  ...
  Future<void> _getSavedFiles() async {
    try {
      ...
      var response = await http.get(
        Uri.parse('http://10.0.2.2:8080/api/trash/files/user/$userId'),
      );
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Column(
            children: savedFiles
                .map((file) => FileContainer(
              file: file,
              refreshFiles: _getSavedFiles, // Pass the refresh function
            ))
                .toList(),
          ),
        ],
...
class FileContainer extends StatelessWidget {
  final FileData file;
  final Function refreshFiles; // Callback function

  FileContainer({required this.file, required this.refreshFiles});
  ...
  void _viewPDFContent(BuildContext context, FileData file) async {
    try {
      var response = await http.get(
        Uri.parse('http://10.0.2.2:8080/api/trash/files/${file.id}/content'),
      );
